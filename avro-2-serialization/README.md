...
- **avro-2-producer-service**
  ...
  - Run application
    ```
    ./mvnw clean package spring-boot:run --projects avro-2-serialization/avro-2-producer-service -DskipTests
    ```
...
- **avro-2-consumer-service**
  ...
  - Run application
    ```
    ./mvnw clean package spring-boot:run --projects avro-2-serialization/avro-2-consumer-service -DskipTests
    ```
    > The Java class `com.mycompany.commons.avroserialization.avro.NewsMessage` is generated by the Avro file `news-message.avsc` present in `src/main/resources/avro` by running the command
    > ```
